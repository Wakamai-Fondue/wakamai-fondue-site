  ...
  <section class="summary content" id="summary">
    ...
    <p class="oneliner">
      ...
      <template v-if="font.isColor">
        and
        <strong> {{ this.font.colorFormats | listify }} color glyphs. </strong>
      </template>
      ...
      <template v-if="font.isVariable">
        <strong> {{ axesCount }} {{ axesCount | pluralize("axe") }} </strong>
        and
        <strong
          >{{ instancesCount }} {{ instancesCount | pluralize("instance") }},
        </strong>
      </template>
    ...
    <table class="details">
      <tbody>
        <tr v-for="(value, key) in summary" :key="key">
          <th scope="row">{{ key }}</th>
          <td v-html="value" v-linkified:options="{ className: '' }" />
        </tr>
      </tbody>
    </table>
    ...
    <div v-if="isExamplefont" class="example-font-shoutout">
      ...
      <a href="https://djr.com/gimlet/#buy" target="_blank">Get it here!</a>
    </div>
  </section>
</template>
